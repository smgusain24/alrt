...
def deliver(self, execution_id, subscriber_id, team_id, template_data, payload,
            notification_id=None, overrides=None, platform=None, subscriber=None):
    ...
    provider = get_provider(team_id, "push", lambda: execute_read_one_query(Q_GET_PUSH_PROVIDER, [uuid.UUID(team_id)]))
    if not provider:
